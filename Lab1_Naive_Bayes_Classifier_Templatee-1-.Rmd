...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    score = function(X_test, y_test)
                    {
                      ...
                      good_predictions = 0
                      for (i in seq(1,len)) {
                        if (X_test[i,3]==predict(y_test[i,1])) {
                          good_predictions <- good_predictions + 1
                        }
                      }
                      good_predictions<<-good_predictions
                      results = good_predictions/len
                      return(results)
                    }
))
...
```
```{r}
x <-  c(len-good_predictions, good_predictions)
...
# Plot the chart.
pie(x, labels = piepercent, main = "Persentage of success",col = rainbow(length(x)))
legend("bottomright", c("Wrong ansver", "Correct ansver"), cex = 1,
